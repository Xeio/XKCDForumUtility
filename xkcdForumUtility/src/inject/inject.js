...
function makeQuoteLinksPrettier() {
    //Transforms any quote arrow link into a link on the quote text
    Array.from(document.querySelectorAll("blockquote > div > cite + a")).forEach(function (aElement) {
        if (aElement.text === "↶") {
            var citeElement = aElement.parentElement.children[0];
            aElement.parentElement.querySelector("cite + a + br").remove();
            aElement.remove();
            aElement.text = citeElement.textContent;
            citeElement.textContent = "";
            citeElement.appendChild(aElement);
        }
    });
}
